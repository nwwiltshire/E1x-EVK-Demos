...
reports effective throughput plus the meter-reader frame rate that
implies.  Step 1 of hardware bring-up (see README): prove the port,
baud, and byte integrity before flashing the real firmware.

  python3 host/echo_test.py --sim 127.0.0.1:5555          # simulator
  python3 host/echo_test.py --port /dev/ttyACM2 --baud 115200
"""
...
FRAME_BYTES = 4096  # one 64x64 frame
CHUNKS = 8

# bytes on the wire per frame exchange (see protocol.h)
DEV_BYTES = (FRAME_BYTES + 8) + (480 + 8) + 21 + 7  # frame + scores + status + ack
DEPLOY_BYTES = (FRAME_BYTES + 8) + 21 + 7
...
def main() -> None:
    ...
    # each chunk travels down AND back within dt, so the one-way line
    # rate on this half-duplex link is twice the round-trip figure
    rate = 2 * kbs * 1000
    print(f"\necho ok: {args.chunks}x{FRAME_BYTES} bytes, {kbs:.1f} KB/s round-trip")
    print(f"  -> ~{rate / DEV_BYTES:.2f} fps DEV (frame down + scores/status/ack up)")
    print(f"  -> ~{rate / DEPLOY_BYTES:.2f} fps DEPLOY (frame down + status/ack up)")
